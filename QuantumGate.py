...
class QuantumGate(object):
    def __init__(self):
        ...
        self.O1 = np.array([[1, 0], [0, 0]]) #0控制位
        self.O2 = np.array([[0, 0], [0, 1]]) #1控制位

    # n表示控制比特和目标比特之间隔了n个qubit,当控制比特为1是，U作用到目标比特,高位控制低位
    def C1nU(self,n=0, U=np.array([[0,1],[1,0]])):
        a = int(np.log2(U.shape[0]))  # U操作的量子比特数
        zero = np.kron(self.O1, np.eye(2 ** (n + a))) # 非激活部分
        one = np.kron(self.O2, np.kron(np.eye(2 ** n), U))
        return zero + one  # 返回控制门的矩阵表示
...
#在循环中，用np.kron函数计算所有量子门的张量积，构造该层的整体表示。
    def layer(self,n_qubit,gate_List):
        temp = 1
        for gate in gate_List:
            temp = np.kron(temp,gate)  # 通过Kronecker积来叠加量子门
